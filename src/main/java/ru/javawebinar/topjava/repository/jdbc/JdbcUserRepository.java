...
//        jdbcTemplate.query("select u.name, u.email, string_agg(ur.role, ','), u.registered from users u left join user_roles ur\n" +
//                "    on u.id = ur.user_id group by u.name, u.email, u.registered order by u.name, u.email", ROW_MAPPER);
    ...
    private void insertBatchRoles(User user) {
